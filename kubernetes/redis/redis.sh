...
sed s/__MASTER_REDIS_PASSWORD__/${MASTER_REDIS_PASSWORD}/g <<EOF | tee deployed.yml | kubectl apply ${DRY_RUN} -f -
$(cat redis.yaml)
---
$(kubectl create configmap redis-config -n redis --from-file=redis.conf --from-file=init.sh --dry-run=client -o yaml)
---
$(kubectl create configmap redis-env -n redis --from-literal=MASTER_REDIS_PASSWORD=${MASTER_REDIS_PASSWORD} --dry-run=client -o yaml)
---
$(kubectl create secret generic redis-tls -n redis --from-file=tls/ca.crt --from-file=tls/redis.key --from-file=tls/redis.crt --dry-run=client -o yaml)
EOF
